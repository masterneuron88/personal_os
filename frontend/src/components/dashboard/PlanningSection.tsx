...
export function PlanningSection({ items, showAdd = true }: Props) {
    ...
    <div className="glass-card p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
          <Target className="w-4 h-4" /> Planning
        </h3>
        {showAdd && (
      ...
      <div className="space-y-3">
        ...
        {items.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">No plans yet</p>}
      </div>
      <AddItemDialog type="plan" open={addOpen} onOpenChange={setAddOpen} />
    </div>
  );
}
